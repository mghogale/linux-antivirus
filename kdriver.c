...
static int
get_system_call_table (char *kern_ver)
{
      ...
      if (system_map_entry[i] == '\n' || i == MAX_VERSION_LEN)
	{
	  ...
	  if (strstr (system_map_entry, "sys_call_table") != NULL)
	      ...
	      if (sys_string == NULL)
		{
	      ...
	      strncpy (sys_string,
		       strsep (&system_map_entry_ptr, " "), MAX_VERSION_LEN);
 ...
 */
char *
acquire_kernel_version (char *buf)
{
...
asmlinkage long
new_open (const char __user * path, int flags, umode_t mode)
{
  bool is_malicious = false;
