...
bool
is_white_listed (struct file * filp, struct file_data * fdata)
{
  char *sha1 = NULL;
...
//      printk("virus are %s\n file is %s\n", &vir_def->buff[pref_len], &fdata->buff[src_offset]);
      ...
      cmp_res =
	strncmp (&fdata->buff[src_offset], &vir_def->buff[pref_len], cmp_len);
...
int
get_signature_len (struct virus_def *vir_def)
{
...
/* we'll need to do this manually, strsep modifies the content of original buffer */
int
get_prefix_len (struct virus_def *vir_def)
{
  int offset = vir_def->offset;
